...
def create_gradio_app(config: QORConfig, checkpoint_path: str):
    """
    Create an enhanced Gradio web interface for QOR.
    Features: chat UI, confidence display, source display, generation settings.
    """
    try:
        import gradio as gr
    except ImportError:
    ...
    def chat_respond(message, history, temperature, max_tokens, top_k):
        ...
        response = result.get('generated_text', result.get('output', ''))
        stats = f"Tokens: {result['tokens_generated']} | Time: {result['time_seconds']}s | Speed: {result['tokens_per_second']} tok/s"
        return response, stats

    def clear_chat():
        if hasattr(server.model, 'reset_fast_weights'):
            server.model.reset_fast_weights()
        return [], "", ""

    with gr.Blocks(title="QOR — The Qore Mind", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# QOR — The Qore Mind")
        gr.Markdown(f"A {server.model.n_params/1e6:.1f}M parameter model with multi-speed memory and self-modifying neurons.")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                stats_display = gr.Textbox(label="Generation Stats", interactive=False, lines=2)
                ...
                gr.Markdown(f"- **Parameters**: {info['parameters']:,}\n"
                           f"- **Device**: {info['device']}\n"
                           f"- **CMS Levels**: {info['cms_levels']}")

        def user_submit(message, history, temperature, max_tokens, top_k):
            if not message.strip():
                return "", history, ""
            history = history + [{"role": "user", "content": message}]
            response, stats = chat_respond(message, history, temperature, max_tokens, top_k)
            history = history + [{"role": "assistant", "content": response}]
            return "", history, stats

        send_btn.click(
            user_submit,
            [msg, chatbot, temperature, max_tokens, top_k],
        ...
        clear_btn.click(clear_chat, [], [chatbot, msg, stats_display])
