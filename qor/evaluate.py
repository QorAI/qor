...
def bleu_score(reference: str, hypothesis: str, max_n: int = 4) -> float:
    """Compute BLEU score (n-gram precision with brevity penalty). Pure Python."""
    ...
    return bp * math.exp(log_precision / n_valid)


def rouge_l_score(reference: str, hypothesis: str) -> float:
    """Compute ROUGE-L (Longest Common Subsequence) F1 score. Pure Python."""
    ref_tokens = reference.lower().split()
    hyp_tokens = hypothesis.lower().split()
...
class Evaluator:
    ...
    def full_report(self, checkpoint_path: str,
        ...
        # BLEU/ROUGE (if reference texts provided)
        if eval_text_path and os.path.exists(eval_text_path):
            with open(eval_text_path, 'r', encoding='utf-8', errors='replace') as f:
                ref_text = f.read()
            # Generate a continuation from the start of the reference text
            ref_prompt = ' '.join(ref_text.split()[:20])
            ref_ids = tokenizer.encode(ref_prompt, add_special_tokens=True)
            ref_input = torch.tensor([ref_ids], device=self.device)
            gen_ids = model.generate(ref_input, max_new_tokens=100, temperature=0.5)
            gen_text = tokenizer.decode(gen_ids, skip_special_tokens=True)
            ref_snippet = ' '.join(ref_text.split()[:120])
            report["bleu"] = round(bleu_score(ref_snippet, gen_text), 4)
            report["rouge_l"] = round(rouge_l_score(ref_snippet, gen_text), 4)
            print(f"  BLEU:     {report['bleu']:.4f}")
            print(f"  ROUGE-L:  {report['rouge_l']:.4f}")
